def RK(tsim, dt, x0, delta):
    ...
    k_1 = model(tsim_1, x0, delta)
    k_2 = model(tsim_1 + 0.5 * h, x0 + 0.5 * h * k_1, delta)
    k_3 = model(tsim_1 + 0.5 * h, x0 + 0.5 * h * k_2, delta)
    k_4 = model(tsim_1 + h, x0 + k_3 * h, delta)
...
def model(t, x, u):
    ...
    # Rolling force
    froll = hev.Ambient.g * hev.Chassis.m * hev.Chassis.crr * np.cos(theta) * vhev
    # Gravity force
    ...
    soc_dot = -hev.Battery.n * im * abs(delta_t) / hev.Battery.q  # SOC [%]
    ...
    # Derivative Vector
    xdot = np.array([vhev_dot, phev_dot, iind_dot, im_dot, um_dot, soc_dot, wice_dot, i_r1_dot]).T
